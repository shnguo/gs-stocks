"""Compare dated retrospective industry context on immutable plan cohorts."""
import argparse
import json
import os
import shutil
...
from quant_research.industry_context import INDUSTRY_FEATURES, industry_context, read_snapshot
...
def freeze(args):
    ...
    snapshot_index = read(args.industry/'verified-snapshots.json')
    ...
    source_dates = {d: d for d in required}
    if protocol.get('snapshot_cadence') == 'weekly-earliest-required-day':
        source_dates = read(args.industry/'target-source-map.json')
        anchors = {}
        expected = {}
        for day in sorted(required):
            key = pd.Timestamp(day).isocalendar()[:2]
            anchors.setdefault(key, day)
            expected[day] = anchors[key]
        if source_dates != expected or protocol.get('max_snapshot_age_days') != 6:
            raise ValueError('Weekly mapping differs from preregistered past-only source dates')
    if set(snapshot_index) != set(source_dates.values()):
        raise ValueError('Incomplete or differently scoped industry date index')
    evidence = {}
    for date, source_date in source_dates.items():
        directory = snapshot_index[source_date]
        read_snapshot(directory, source_date)
        evidence[date] = dict(directory=directory, source_date=source_date,
            manifest_sha256=file_hash(Path(directory)/'manifest.json'))
    out.mkdir()
    ...
    for name in ['industry_ablation_run.py', 'plan_value_run.py', 'price_pilot.py', 'model_quality_run.py']:
...
def fit(args):
    ...
    write_json(dest/'features.json', {'base': names, 'added': [INDUSTRY_FEATURES[i] for i in columns],
        'source': 'retrospective historical snapshots; not certified publication-time history'})
    ...
    def load(part):
        ...
        for date in sorted(rows.date.unique()):
            ...
            snapshots[date] = read_snapshot(directory, entry.get('source_date', date))
        context = industry_context(data['x'], rows, names, snapshots, protocol['minimum_peers'],
            protocol.get('max_snapshot_age_days', 0))[:, columns]
        write_json(dest/f'{part}-industry-coverage.json', dict(total_rows=len(rows),
            usable_industry_rows=int((context[:, 0] == 1).sum()),
            missing_or_small_peer_rows=int((context[:, 0] == 0).sum())))
        ...
        for fold in experiment['training_protocol']['fold_indices']:
            for arm in protocol['arms']:
                ...
                with (out/f'{name}.log').open('w') as log:
                    subprocess.run([sys.executable, str(out/'code/scripts/industry_ablation_run.py'),
